...
/*
 * shiroikuma-termux (Phase 4c): Termux:Widget absorbed into the app.
 *
 * Upstream: https://github.com/termux/termux-widget — fetch-only remote `upstream-widget`, ported
 * from commit b3aacb8175fb2d7167ce8d1ad6817087498aa044 (2026-01-21, "Fixed: Enable
 * `desugar_jdk_libs` at 1.1.5"). The package `com.termux.widget` is kept so an upstream diff ports
 * by hand; every change against upstream is marked "shiroikuma-termux". Package-wide:
 *  - TermuxWidgetApplication is no longer an Application (this process has TermuxApplication) —
 *    only its setLogConfig() stub remains so the call sites stay as upstream wrote them;
 *  - activities/TermuxWidgetMainActivity (info page, launcher-icon alias) is dropped: no second
 *    launcher icon; its dynamic-shortcut logic moved to utils/ShortcutUtils and is driven from the
 *    白い熊 Termux UI page's Widget section (Refresh widgets · Create / Remove dynamic shortcuts);
 *  - R.drawable.ic_launcher → the app's R.mipmap.ic_launcher; the widget's strings live in
 *    res/values/termux_widget_strings.xml with our names; the plugin help URL is ours;
 *  - the list's colours / text size come from ShiroikumaTheme (widget_bg, widget_text,
 *    widget_text_size_sp) and are applied to the RemoteViews on every refresh.
 * Token and log level are read through TermuxWidgetAppSharedPreferences, whose build() now
 * resolves to this app's own preferences when the caller is com.termux (termux-shared change).
 */
...
public final class TermuxWidgetProvider extends AppWidgetProvider {
    ...
    public static void updateAppWidgetRemoteViews(@NonNull Context context, @NonNull AppWidgetManager appWidgetManager, int appWidgetId) {
        ...
        // shiroikuma-termux: the Widget section of the 白い熊 Termux UI page — list background, text
        // colour (title, empty text, refresh icon) — re-applied on every refresh.
        int widgetBg = ShiroikumaTheme.color(context, ShiroikumaTheme.WIDGET_BG);
        int widgetText = ShiroikumaTheme.color(context, ShiroikumaTheme.WIDGET_TEXT);
        remoteViews.setInt(R.id.top_row, "setBackgroundColor", widgetBg);
        remoteViews.setInt(R.id.widget_list, "setBackgroundColor", widgetBg);
        remoteViews.setInt(R.id.empty_view, "setBackgroundColor", widgetBg);
        remoteViews.setTextColor(R.id.widget_title, widgetText);
        remoteViews.setTextColor(R.id.empty_view, widgetText);
        remoteViews.setInt(R.id.refresh_button, "setColorFilter", widgetText);
    ...
    public static void sendExecutionIntentToTermuxService(final Context context, String shortcutFilePath, String logTag) {
        ...
        executionIntent.putExtra(TERMUX_SERVICE.EXTRA_PLUGIN_API_HELP, context.getString(R.string.plugin_api_help, TermuxConstants.TERMUX_GITHUB_REPO_URL)); // shiroikuma-termux: our repo
