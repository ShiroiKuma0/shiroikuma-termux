...
public class TermuxFloatView extends LinearLayout {
    ...
    public void initFloatView(TermuxFloatService service) {
        ...
        initWindowControls();
        applyWindowStyle(false); // shiroikuma-termux
    }

    /**
     * shiroikuma-termux (Phase 4c): the Floating terminal section of the 白い熊 Termux UI page —
     * window background, border colour / width (0–4 dp; the border becomes the frame's padding so
     * the terminal never covers it) and corner radius (0–24 dp; children are clipped to the rounded
     * outline). Replaces upstream's static floating_window_background(.xml) and, while resizing
     * ({@code resizing}), its red-stroked _resize variant. The controls bar takes the window colour
     * and its two icons the border colour.
     */
    public void applyWindowStyle(boolean resizing) {
        Context ctx = getContext();
        float density = ctx.getResources().getDisplayMetrics().density;
        int bg = ShiroikumaTheme.color(ctx, ShiroikumaTheme.FLOAT_BG);
        int border = ShiroikumaTheme.color(ctx, ShiroikumaTheme.FLOAT_BORDER_COLOR);
        int borderPx = Math.round(ShiroikumaTheme.value(ctx, ShiroikumaTheme.FLOAT_BORDER_DP) * density);
        int radiusPx = Math.round(ShiroikumaTheme.value(ctx, ShiroikumaTheme.FLOAT_CORNER_DP) * density);

        GradientDrawable frame = new GradientDrawable();
        frame.setShape(GradientDrawable.RECTANGLE);
        frame.setColor(bg);
        frame.setCornerRadius(radiusPx);
        if (resizing) frame.setStroke(Math.max(1, borderPx), 0xFFFF4444);
        else if (borderPx > 0) frame.setStroke(borderPx, border);
        setBackground(frame);
        setPadding(borderPx, borderPx, borderPx, borderPx);
        setClipToOutline(radiusPx > 0);

        if (mWindowControls != null) {
            mWindowControls.setBackgroundColor(bg);
            for (int id : new int[]{R.id.minimize_button, R.id.exit_button}) {
                View button = mWindowControls.findViewById(id);
                if (button != null && button.getBackground() != null) {
                    button.getBackground().mutate().setTint(border);
                }
            }
        }
    }
